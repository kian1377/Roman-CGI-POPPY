...
def run_model(npix=1024,
    ...
    if mode=='SPC730':
        ...
        opddir = Path('/groups/douglase/webbpsf-data/CGI/OPD-spc730')
        ...
        opddir = Path('/groups/douglase/webbpsf-data/CGI/OPD-spc825')
        ...
        opddir = Path('/groups/douglase/webbpsf-data/CGI/OPD-hlc575')
    ...
    # initialize the mode specific elements
    if mode=='SPC730' or mode=='SPC825':
        ...
        if use_fpm:
            FPM = poppy.FITSFPMElement(fpm_name, fpm_fname, 
                                       wavelength_c=lambda_c_m, ep_diam=D, pixelscale_lamD=fpm_pxscl_lamD, centering=centering,)
        else:
...
#                                                transmission=dm2_mask_fname, 
                                               opd=dm2_fname, opdunits='meters', 
    ...
    fig=plt.figure(figsize=(10,4)); LS.display(what='both'); plt.close(); display(fig)
    ...
    fosys = poppy.FresnelOpticalSystem(name=mode, pupil_diameter=D,
    ...
    if use_errors: fosys.add_optic(secondary_opd)

    fosys.add_optic(fold1, distance=d_sec_fold1)
    if use_errors: fosys.add_optic(fold1_opd)
    ...
    if use_errors: fosys.add_optic(m3_opd)
    ...
    if use_errors: fosys.add_optic(m4_opd)
    ...
    if use_errors: fosys.add_optic(m5_opd)

    fosys.add_optic(fold2, distance=d_m5_fold2)
    if use_errors: fosys.add_optic(fold2_opd)

    fosys.add_optic(fsm, distance=d_fold2_fsm)
    if use_errors: fosys.add_optic(fsm_opd)
    ...
    if use_errors: fosys.add_optic(oap1_opd)

    fosys.add_optic(focm, distance=d_oap1_focm)
    if use_errors: fosys.add_optic(focm_opd) 
    ...
    if use_errors: fosys.add_optic(oap2_opd)

    fosys.add_optic(dm1, distance=d_oap2_dm1)
    if use_errors: fosys.add_optic(dm1_opd)

    fosys.add_optic(dm2, distance=d_dm1_dm2)
    if use_errors: fosys.add_optic(dm2_opd) 
    ...
    if use_errors: fosys.add_optic(oap3_opd)

    fosys.add_optic(fold3, distance=d_oap3_fold3)
    if use_errors: fosys.add_optic(fold3_opd) 
    ...
    if use_errors: fosys.add_optic(oap4_opd) 

    fosys.add_optic(SPM, distance=d_oap4_pupilmask)
    if use_errors: fosys.add_optic(spm_opd) 

    fosys.add_optic(oap5, distance=d_pupilmask_oap5)
    if use_errors: fosys.add_optic(oap5_opd) 
    ...
    if use_errors: fosys.add_optic(oap6_opd)
    ...
    if use_errors: fosys.add_optic(oap7_opd)
    ...
    if use_errors: fosys.add_optic(oap8_opd)

    fosys.add_optic(filt, distance=d_oap8_filter)
    if use_errors: fosys.add_optic(filter_opd)

    fosys.add_optic(lens, distance=d_filter_lens)
    if use_errors: fosys.add_optic(lens_opd)

    fosys.add_optic(fold4, distance=d_lens_fold4)
    if use_errors: fosys.add_optic(fold4_opd)
    ...
    print('FPM shape and pixelscale: ', fpm_wf.shape, fpm_samp, FPM.pixelscale)
...
def make_inwave(mode, D, lambda_c_m, lambda_m, npix, oversample, offsets, polaxis, display=True):
    ...
    if display:
        misc.myimshow2(np.abs(wfin.wavefront)**2, np.angle(wfin.wavefront),
                       ...
                       pxscl=wfin.pixelscale, cmap2='coolwarm') # coolwarm, Spectral
...
def compare_psfs(pop_psf, mode, 
    ...
    innwa = iwa/prop_pxscl_lamD*prop_pxscl # inner and outer working angles in units of m
    outwa = owa/prop_pxscl_lamD*prop_pxscl
    ...
    patches2 = [Circle((0,0),innwa,edgecolor='c', facecolor='none',lw=1),
                Circle((0,0),outwa,edgecolor='c', facecolor='none',lw=1)]
    misc.myimshow2(np.abs(pop_int - prop_int), pop_phs-prop_phs, 'PSF Intesnity Difference', 'PSF Phase Difference',
                   pxscl=prop_pxscl*u.m/u.pix,
                   cmap1='gist_heat', cmap2='magma',
                   lognorm1=True, vmax1=vmax,
                   patches1=patches1, patches2=patches2)
    

def save_waves(mode, wfs, use_apertures, use_errors):
    clear_output()
    if mode=='SPC730':
        wfdir = '/groups/douglase/spc730-fresnel-wavefronts/'
    elif mode=='SPC825':
        wfdir = '/groups/douglase/spc825-fresnel-wavefronts/'
    elif mode=='HLC575':
        wfdir = '/groups/douglase/hlc575-fresnel-wavefronts/'

    if use_apertures==False and use_errors==False:
        optics = ['pupil', 'primary', 'secondary', 'fold1', 'm3', 'm4', 'm5', 'fold2', 'fsm', 'oap1', 
                  'focm', 'oap2', 'dm1', 'dm2', 'oap3', 'fold3', 'oap4', 'spm', 'oap5', 'fpm', 'oap6',
                  'lyotstop', 'oap7', 'fieldstop', 'oap8', 'filter', 'lens', 
                  'fold4', 
                  'image']
        print('Saving wavefronts: ')
        for i,wf in enumerate(wfs):
            n = 2000
            wavefront = misc.trim(wf.wavefront, n)
    ...
    elif use_apertures==False and use_errors==True:
        optics = ['pupil', 
                  'primary', 'primary_opd', 'g20_opd', 
